...
(async function(){
  var sel = $selectorJs;
  var includeDetails = $detailsJs;
  var timeoutMs = $timeoutJs;
  var started = Date.now();
$domSupport
  var root = document.querySelector(sel);
  if (!root) {
    return { ok: false, reason: 'not_found', recovery: silmarilCollectRecoveryCandidates(document, sel, 'any', 8) };
  }
  function nodeInfo(node){
    if (!node) return null;
    if (node.nodeType === 3) {
      var textVal = typeof node.data === 'string' ? node.data : '';
      return { kind: 'text', preview: textVal.slice(0, 120) };
    }
    if (node.nodeType === 1) {
      var desc = silmarilDescribeElement(node);
      return { kind: 'element', name: desc.tag, selector: desc.selector };
    }
  ...
  return await new Promise(function(resolve){
    ...
    observer = new MutationObserver(function(records){
      ...
      var payload = { ok: true, elapsedMs: Date.now() - started, mutationType: first && first.type ? first.type : 'unknown' };
      if (includeDetails && first) {
        var detail = { target: nodeInfo(first.target) };
        if (first.type === 'attributes') {
          detail.attributeName = first.attributeName || null;
          detail.oldValue = first.oldValue == null ? null : String(first.oldValue);
          if (first.target && first.target.getAttribute && first.attributeName) {
            var newAttr = first.target.getAttribute(first.attributeName);
            detail.newValue = newAttr == null ? null : String(newAttr);
          }
        } else if (first.type === 'childList') {
          detail.addedCount = first.addedNodes ? first.addedNodes.length : 0;
          detail.removedCount = first.removedNodes ? first.removedNodes.length : 0;
          detail.firstAdded = (first.addedNodes && first.addedNodes.length) ? nodeInfo(first.addedNodes[0]) : null;
          detail.firstRemoved = (first.removedNodes && first.removedNodes.length) ? nodeInfo(first.removedNodes[0]) : null;
        } else if (first.type === 'characterData') {
          detail.oldValue = first.oldValue == null ? null : String(first.oldValue).slice(0, 200);
          var newText = first.target && typeof first.target.data === 'string' ? first.target.data : '';
          detail.newValue = String(newText).slice(0, 200);
        }
        payload.detail = detail;
      }
      done(payload);
    });
    timer = setTimeout(function(){ done({ ok: false, reason: 'timeout', elapsedMs: Date.now() - started }); }, timeoutMs);
    try {
      observer.observe(root, { subtree: true, childList: true, attributes: true, characterData: true, attributeOldValue: includeDetails, characterDataOldValue: includeDetails });
    } catch (e) {
      done({ ok: false, reason: 'observe_failed', message: String((e && e.message) ? e.message : e) });
    }
  });
})()
"@
...
if (($valueProps -contains "ok") -and -not [bool]$value.ok) {
  if (($valueProps -contains "reason") -and $value.reason -eq "not_found") {
    ...
    throw (New-SilmarilSelectorNotFoundStructuredErrorMessage -CommandName "wait-for-mutation" -InputSelector $selectorInput -NormalizedSelector $selector -Recovery $recovery)
  }
  ...
  throw "Timed out waiting for mutation on selector: $selectorInput"
}
...
if (Test-SilmarilJsonOutput) {
  ...
  Write-SilmarilJson -Value (Add-SilmarilTargetMetadata -Data $payload -TargetContext $targetContext) -Depth 20
  exit 0
}

Write-Host "Mutation observed on selector: $selectorInput ($mutationType, $elapsed ms)"
