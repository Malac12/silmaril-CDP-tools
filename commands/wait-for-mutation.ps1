...
if ($RemainingArgs.Count -gt 2) {
  throw "wait-for-mutation supports: no args, ""selector"", --details, or ""selector"" --details"
}
...
foreach ($arg in $RemainingArgs) {
  ...
  if ($arg.StartsWith("--")) {
    throw "Unsupported flag '$arg'. Supported flag: --details"
  }
