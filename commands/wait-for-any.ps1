...
if (($valueProps -contains "ok") -and -not [bool]$value.ok) {
  if (($valueProps -contains "reason") -and [string]$value.reason -eq "invalid_selector") {
    $badSelector = if (($valueProps -contains "selector") -and -not [string]::IsNullOrWhiteSpace([string]$value.selector)) { [string]$value.selector } else { "" }
    $inputSelector = $badSelector
    for ($idx = 0; $idx -lt $normalizedSelectors.Count; $idx += 1) {
      if ([string]$normalizedSelectors[$idx] -eq $badSelector) {
        $inputSelector = [string]$selectors[$idx]
        break
      }
    }

    $detail = if (($valueProps -contains "message") -and -not [string]::IsNullOrWhiteSpace([string]$value.message)) { [string]$value.message } else { "" }
    throw (New-SilmarilSelectorStructuredErrorMessage -CommandName "wait-for-any" -InputSelector $inputSelector -NormalizedSelector $badSelector -DetailMessage $detail)
  }
...
if (($valueProps -contains "matchedVisibleCount") -and $null -ne $value.matchedVisibleCount) {
  $resultData["matchedVisibleCount"] = [int]$value.matchedVisibleCount
}
...
if ($includeCounts -and ($valueProps -contains "visibleCounts") -and $null -ne $value.visibleCounts) {
