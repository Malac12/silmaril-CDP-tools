...
if ($RemainingArgs.Count -ne 1) {
  throw "wait-for requires exactly one selector argument."
}
...
if (-not $evalResult) {
  throw "No wait-for result returned from CDP."
}
  ...
  if (($runtimeResult -is [System.Collections.IEnumerable]) -and -not ($runtimeResult -is [string])) {
    foreach ($item in @($runtimeResult)) {
      ...
      if ($itemProps -contains "value") {
        ...
        Write-SilmarilCommandResult -Command "wait-for" -Text "Selector found: $selector ($elapsed ms)" -Data @{ selector = $selector; elapsedMs = $elapsed } -UseHost
        exit 0
      }
